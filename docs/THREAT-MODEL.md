...
| Password brute-force / credential stuffing | PBKDF2-100k, login throttle 5/15 min per IP, generic `INVALID_CREDENTIALS`, owner-only registration mode | **Low** — distributed botnets share the same per-IP budget; add Turnstile on login for high-risk tenants |
...
| Spam / bot abuse | Honeypot, proof-of-work, blocklist (+100 score), Turnstile, MX validation, 60/min submit throttle, retention purge | **Low** — determined human farms still pass; pair with Turnstile for public forms |
